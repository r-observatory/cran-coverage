...
# --- manifest integrity / completeness core --------------------------------
# Lets a downstream merge content-verify the canonical database it pulls
# (cran-coverage.db) and confirm the expected tables/rows are present.
...
#'   3. sha256sum (coreutils)  - present on the rocker/r2u noble container
#'   4. shasum -a 256 (BSD)    - macOS/local fallback
#' No heavy dependency is declared: the merge job installs only DBI, RSQLite
#' and jsonlite, so on CI the coreutils sha256sum path is used. If a runner
#' also has digest/openssl, that path wins automatically.
#'
#' @param path Path to the file to hash.
#' @return A single lowercase 64-character hex string.
...
#' Honest completeness of the canonical coverage database.
#'
#' complete = the database holds the full, non-partial dataset: every package
#' in the CRAN universe has been analyzed, so none remain to collect. This
#' pipeline collects incrementally (high-download packages first), so the
#' canonical database is a partial rolling snapshot until `remaining` reaches
#' 0; complete is therefore DERIVED, never hardcoded true. An unavailable
#' universe (`remaining` NA) is treated as not-complete. Freshness is a
#' separate concern, tracked via the manifest generated_at and the db
#' fingerprint (db_sha256), not by this flag.
#'
#' @param remaining Number of packages still to collect (integer, or NA when
#'   the CRAN universe could not be determined this run).
#' @return A single TRUE/FALSE.
coverage_complete <- function(remaining) isTRUE(remaining == 0)
...
#'   * complete    - passed through by the caller (see coverage_complete()):
#'                   the db holds the full, non-partial dataset. Freshness is
#'                   tracked separately via generated_at and db_sha256, not by
#'                   this flag.
#'
#' The table enumeration and row counts are read first; the connection is then
#' closed BEFORE db_bytes/db_sha256 read the raw file, so no open handle or
#' journal file skews the size or hash.
#'
#' @param db_path  Path to the finalized SQLite database.
#' @param complete Honest completeness flag (from coverage_complete()).
#' @return Named list of the integrity/completeness core fields.
